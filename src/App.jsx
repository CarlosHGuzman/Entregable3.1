...
import rickMorty from "./assets/textRM.png" 
    ...
    <div className="App">
      {
        location?(
          <>           
            <header>
              <div className="background-img">
                ...
                <form onSubmit={handleSubmit}>
                  <input id="Id" type="text" onChange={handleSuggestion} list="suggestionDimension" placeholder='Dimension (1-126 or Name)'></input>
                  <datalist id="suggestionDimension">
                    {suggestions?.map(sug => <option key={sug.id}>{sug.name}</option>)}
                  </datalist>
            ...
            <div className="content-dimension">
              <LocationInfo location={location}/>
              <div className="card-resident">
                {
                  residentsPage?.map(urlResident => (
                    <CardResident key={urlResident} urlResident={urlResident}/>
                  ))
                }
              </div>
            </div>
                ...
                <ul>
                  {
                    getAllPages().map(page => (
                      <a href='#' key={page}><li onClick={() => setCurrentPage(page)}
